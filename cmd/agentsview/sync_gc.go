...
// logged, not fatal. Non-folder targets are skipped because this operation
// cannot prune their remote history; collecting only the local store would make
// the next set-union exchange download every remote-only artifact again.
func autoGCAfterFolderSync(ctx context.Context, dataDir, target string, grace time.Duration) {
	...
	roots := []string{filepath.Join(dataDir, "artifacts"), target}
