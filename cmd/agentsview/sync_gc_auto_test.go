...
func TestAutoGCAfterFolderSyncSkipsLocalGCForNonFolderTarget(t *testing.T) {
	for _, tc := range []struct {
		name   string
		target string
	}{
		{name: "http", target: "https://peer.example.test:8080"},
		{name: "s3", target: "s3://bucket/artifacts"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			// A non-folder target cannot be pruned by the same operation. Use a
			// placeholder folder for its remote history: neither side may be
			// collected, or the next set-union exchange would download the
			// remote-only files again.
			untouched := t.TempDir()
			dataDir, origin := autoGCTestStore(t, untouched)
			localOriginRoot := filepath.Join(dataDir, "artifacts", origin)
			untouchedOriginRoot := filepath.Join(untouched, origin)

			autoGCAfterFolderSync(context.Background(), dataDir, tc.target, time.Hour)

			// Both stores retain the superseded set until a transport can prune
			// them together.
			require.Equal(t, 2, countFiles(t,
				filepath.Join(localOriginRoot, artifact.KindSegments)))
			require.Equal(t, 2, countFiles(t,
				filepath.Join(untouchedOriginRoot, artifact.KindSegments)))
		})
	}
}
