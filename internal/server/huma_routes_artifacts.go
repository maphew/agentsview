...
func (s *Server) artifactCapableRoot() (string, error) {
...
func (s *Server) humaListArtifactOrigins(
	...
	root, err := s.artifactCapableRoot()
	if err != nil {
		return nil, err
	}
	if err := s.publishLocalArtifacts(ctx, root); err != nil {
		return nil, err
	}
...
func (s *Server) publishLocalArtifacts(ctx context.Context, root string) error {
	...
	if _, err := artifact.Export(ctx, local, root, origin); err != nil {
...
func (s *Server) humaGetArtifact(
	...
	art, err := artifact.ReadArtifactForServe(root, in.Origin, in.Kind, in.Name)
	if err != nil {
...
func (s *Server) humaPostArtifact(
	...
	s.lockSessionLifecycle()
	defer s.sessionLifecycleMu.Unlock()
	if res.Kind == artifact.KindCheckpoints || res.Kind == artifact.KindMeta || s.artifactImportPending {
		importRes, err := s.importPeerArtifacts(ctx, local, root)
		if err != nil {
			return nil, err
		}
...
func (s *Server) importPeerArtifacts(
	ctx context.Context, local *db.DB, root string,
) (artifact.ImportResult, error) {
	var res artifact.ImportResult
	var err error
	if s.metadata != nil {
		// An incoming peer exchange is this machine's opt-in to artifact
		// sync, so mint the origin here. The recorder itself never creates
		// one: ordinary curation stays ledger-free until a machine opts in.
		if s.cfg.ArtifactOriginID == "" {
			if _, err := artifact.EnsureOrigin(local); err != nil {
