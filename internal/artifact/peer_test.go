...
func TestCheckpointSummarySkipsAndQuarantinesCorruptLatestCheckpoint(t *testing.T) {
	...
	latest := filepath.Join(root, origin, "checkpoints", "cp-0000000002.json")
	require.NoError(t, os.WriteFile(latest, []byte("not json"), 0o644))

	// The summary falls back to the newest checkpoint that still decodes and
	// quarantines the corrupt one, instead of failing the peers status view.
	summary, err := CheckpointSummary(root, origin)
	require.NoError(t, err)
	...
	assert.FileExists(t, latest+quarantineSuffix)
}
