...
func TestMetadataRecorderWithoutOriginIsNoOp(t *testing.T) {
	database := testDB(t)
	dataDir := t.TempDir()
	recorder := NewMetadataRecorder(database, MetadataRecorderOptions{DataDir: dataDir})
	...
	_, err = os.Stat(filepath.Join(dataDir, "artifacts"))
	assert.True(t, os.IsNotExist(err),
		"no artifact store should be created without an origin")
