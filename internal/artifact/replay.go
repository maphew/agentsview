...
func replayMetadata(
	...
	appliedEvents map[db.MetadataEventIdentity]struct{},
) (int, error) {
	events, err := readMetadataArtifacts(originRoot, origin, appliedEvents)
	if err != nil {
		return 0, err
	}
	changed := 0
	for _, art := range events {
		...
		stamp, perr := ParseHLCTimestamp(art.hlc)
		if perr != nil {
			// A future format version may change the HLC shape; defer such
			// events for an upgraded app instead of discarding them.
			if art.event.Version > formatVersion {
				continue
			}
			// LWW ordering is a raw order-key comparison, so an event whose
			// HLC does not parse could sort above every real timestamp and
			// win permanently. The artifact is immutable; mark it applied so
			// it can never enter replay ordering.
			log.Printf("artifact: skipping metadata event %s with invalid HLC: %v", art.path, perr)
		...
		if err != nil {
			// A known op with a malformed payload can never become
			// applicable: the artifact is immutable. Mark it applied so
			// one bad event cannot permanently block the origin's replay.
			log.Printf(
				"artifact: skipping malformed metadata event %s: %v",
				art.path, err,
			)
			if err := database.MarkMetadataEventApplied(ctx, origin, art.orderKey, art.hash); err != nil {
				return changed, err
...
func readMetadataArtifacts(
	originRoot, origin string,
	appliedEvents map[db.MetadataEventIdentity]struct{},
) ([]metadataArtifact, error) {
	...
	for _, path := range paths {
		orderKey, keyErr := metadataArtifactOrderKey(path)
		if keyErr == nil {
			identity := db.MetadataEventIdentity{Origin: origin, OrderKey: orderKey}
			if _, ok := appliedEvents[identity]; ok {
				continue
			}
		}
		...
		if err != nil {
			...
			log.Printf("artifact: skipping unreadable metadata event %s for %s: %v", path, origin, err)
			continue
		}
...
func readMetadataArtifact(path string) (metadataArtifact, error) {
	name, err := metadataArtifactOrderKey(path)
	if err != nil {
		return metadataArtifact{}, err
	}
	idx := strings.LastIndex(name, "-")
	if idx < 0 {
		return metadataArtifact{}, fmt.Errorf("metadata artifact %s missing hash suffix", filepath.Base(path))
	}
	hlc := name[:idx]
	hash := name[idx+1:]
	...
	if got := hashHex(data); got != hash {
		return metadataArtifact{}, fmt.Errorf("%w: metadata artifact %s hash mismatch: got %s", errCorruptArtifact, filepath.Base(path), got)
	}
	var event metadataEvent
	if err := json.Unmarshal(data, &event); err != nil {
		return metadataArtifact{}, fmt.Errorf("decoding metadata artifact %s: %w", filepath.Base(path), err)
	}
