...
func newHTTPTransport(target, token string, allowInsecure bool) (*httpTransport, error) {
	...
	if u.Scheme == "http" && !allowInsecure && !isLoopbackEndpointHost(u.Hostname()) {
		return nil, fmt.Errorf(
			"insecure artifact peer %q requires HTTPS",
			target,
		)
	}
	if u.Scheme == "http" && allowInsecure && !isLoopbackEndpointHost(u.Hostname()) {
		log.Printf(
			"warning: artifact sync to %q uses plaintext HTTP; credentials and archive content are not encrypted in transit",
			target,
		)
	}
...
func (t *httpTransport) Prepare(ctx context.Context, _ string) error {
	if _, err := t.listOrigins(ctx); err != nil {
...
func (t *httpTransport) pull(ctx context.Context, localRoot string) error {
	...
	for _, origin := range origins {
		...
		quarantined, err := reconcileCommonCheckpoint(localRoot, origin, local, remote,
			func(name string) ([]byte, error) {
				return t.getArtifact(ctx, origin, KindCheckpoints, name)
			})
		if err != nil {
			return err
		}
		if quarantined {
			if local, err = ListArtifacts(localRoot, origin); err != nil {
				return err
			}
		}
		for _, item := range missingItems(remote, local) {
			...
			if err != nil {
				if errors.Is(err, ErrArtifactNotFound) {
					log.Printf("artifact: peer no longer serves %s/%s/%s; skipping",
						origin, item.kind, item.name)
					continue
				}
				return err
			}
			if _, err := WriteArtifact(localRoot, origin, item.kind, item.name, data); err != nil {
				if errors.Is(err, ErrArtifactInvalid) {
					log.Printf("artifact: skipping corrupt artifact %s/%s/%s from peer: %v",
						origin, item.kind, item.name, err)
					continue
				}
				return err
			}
...
func (t *httpTransport) push(ctx context.Context, localRoot string) error {
	...
	for _, origin := range origins {
		...
		for _, item := range missingItems(local, remote) {
			...
			if err != nil {
				if skipUnpublishableArtifact(localRoot, origin, item, err) {
					continue
				}
				return err
			}
...
// skipUnpublishableArtifact reports whether a local artifact that failed to
// read before publishing should be skipped: a corrupt file is quarantined so
// a valid copy can be re-fetched, and a file already removed (quarantined or
// collected by a concurrent run) is simply gone. Anything else aborts.
func skipUnpublishableArtifact(localRoot, origin string, item artifactItem, err error) bool {
	if errors.Is(err, ErrArtifactInvalid) {
		log.Printf("artifact: not publishing corrupt artifact %s/%s/%s: %v",
			origin, item.kind, item.name, err)
		quarantineArtifact(filepath.Join(localRoot, origin, item.kind, item.name))
		return true
	}
...
func (t *httpTransport) getArtifact(ctx context.Context, origin, kind, name string) ([]byte, error) {
	...
	if resp.StatusCode == http.StatusNotFound {
		// The peer listed this artifact but no longer serves it, typically
		// because it quarantined a corrupt copy between index and fetch.
		return nil, fmt.Errorf("%w: %s", ErrArtifactNotFound, resp.Status)
	}
