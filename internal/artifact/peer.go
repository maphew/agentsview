...
func CheckpointSummary(root, origin string) (OriginCheckpointSummary, error) {
	...
	paths, err := filepath.Glob(filepath.Join(root, origin, "checkpoints", "cp-*.json"))
	if err != nil {
	...
	for _, path := range slices.Backward(paths) {
		...
		if err := json.Unmarshal(data, &cp); err != nil {
			log.Printf("artifact: skipping corrupt checkpoint %s in peer summary: %v", path, err)
...
func ReadArtifact(root, origin, kind, name string) (PeerArtifact, error) {
	...
	return readArtifactSpec(spec)
}

// ReadArtifactForServe returns one artifact for peer download. A stored file
// that fails content validation is quarantined and reported as not found:
// dropping it from the served set removes it from the origin index, so a peer
// that still holds a valid copy re-uploads it on a later push and the store
// heals, instead of every pull failing against the same bytes forever. A
// request whose origin, kind, or name is itself invalid is still rejected as
// invalid without touching any file.
func ReadArtifactForServe(root, origin, kind, name string) (PeerArtifact, error) {
	spec, err := makePeerArtifactSpec(root, origin, kind, name)
	if err != nil {
		return PeerArtifact{}, err
	}
	art, err := readArtifactSpec(spec)
	if err != nil && errors.Is(err, ErrArtifactInvalid) {
		log.Printf("artifact: quarantining corrupt artifact %s/%s/%s requested by peer: %v",
			origin, kind, name, err)
		quarantineArtifact(spec.path)
		return PeerArtifact{}, ErrArtifactNotFound
	}
	return art, err
}

func readArtifactSpec(spec peerArtifactSpec) (PeerArtifact, error) {
...
func makePeerArtifactSpec(root, origin, kind, name string) (peerArtifactSpec, error) {
	...
	spec, err := artifactSpecForKind(origin, kind, name)
	if err != nil {
		return peerArtifactSpec{}, err
	}
...
// artifactSpecForKind resolves an artifact's kind, canonical name, and
// content hash from its path identity alone, without requiring a valid origin
// id or store root, so on-disk files can be validated in place.
func artifactSpecForKind(origin, kind, name string) (peerArtifactSpec, error) {
...
func validateManifestArtifactData(data []byte, origin, hash string) error {
	decoded, err := readCompressedBytes(data, manifestDecodedLimit)
	if err != nil {
...
func validateSegmentArtifactData(data []byte, hash string) error {
	decoded, err := readCompressedBytes(data, segmentDecodedLimit)
	if err != nil {
...
func readCompressedBytes(data []byte, maxDecoded int64) ([]byte, error) {
	...
	defer dec.Close()
	out, err := io.ReadAll(io.LimitReader(dec, maxDecoded+1))
	if err != nil {
		return nil, err
	}
	if int64(len(out)) > maxDecoded {
		return nil, fmt.Errorf("decoded output exceeds %d-byte limit", maxDecoded)
	}
	return out, nil
}

func isArtifactPathConflict(err error) bool {
	return errors.Is(err, errArtifactPathConflict)
}
